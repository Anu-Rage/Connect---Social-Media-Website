...
import com.connect.service.CommentService;
...
public class CommentController {
	...
	public ResponseEntity<Comments> createCommentHandlerForReels(@RequestBody Comments comment,
																 @PathVariable("reelId") Long reelId,
																 @RequestHeader("Authorization")String token) throws PostException, UserException{
		User user = userService.findUserProfileByJwt(token);
		Comments createdComment = commentService.createCommentReels(comment, reelId, user.getId());
		return new ResponseEntity<Comments>(createdComment,HttpStatus.CREATED);
